<template>
	<HomeLayout>
		...
		<div class="px-5 lg:px-2 w-full lg:w-3/4 xl:max-w-6xl flex flex-row flex-wrap mt-16 mx-auto items-stretch">
			<div class="py-3 w-full lg:w-1/2 lg:px-2 xl:w-1/4" v-for="post in posts.skip(1)" :key="post.id">
				<PostCard :post="post"/>
			</div>
		</div>
	</HomeLayout>
...
export default {
	...
	computed: {
		...
		posts() {
			return collect(this.$page.allPost.edges.map(e => {
				return e.node;
			}));
		}
	},
